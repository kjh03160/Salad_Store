...
class Category(Resource):
    ...
    def get(self):
        ...
        if request['pk'] == None:
            ...
            for i in category:
                return_list.append(
                    {'categoryPk': i.category_pk, 'categoryName': i.category_name})
        else:
            try:
                category = session.query(models.Category).filter(
                    models.Category.category_pk == request['pk']).first()
                return_list.append(
                    {'categoryPk': category.category_pk, 'categoryName': category.category_name})
            except:
                return Response(status=404)
...
class Menu(Resource):
    ...
    parser.add_argument('data', action='append',
                        required=False, help='data is missing')
    ...
    def post(self):
        ...
        if 'image' in request.files:
            ...
            local_path = os.path.join(
                saveImgDir, 'main/', secure_filename(image.filename))
            image.save(local_path)
            server_path = os.path.join(
                serverImgDir, 'main/', secure_filename(image.filename))
    ...
    def patch(self):
        ...
        menu = session.query(models.Menu).filter(
            models.Menu.menu_pk == data['menu_pk']).first()
